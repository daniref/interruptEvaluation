...
def open(device_handle, sampling_frequency=100e06, buffer_size=4096):
    ...
    dwf.FDwfDigitalInDividerSet(device_handle, c_int(1))             #divisore di frequenza
...
def trigger(device_handle, enable, channel, buffer_size=4096, position=0, timeout=0, rising_edge=True, length_min=0, length_max=20, count=1):
    ...
    if enable:
        dwf.FDwfDigitalInTriggerSourceSet(device_handle, trigsrcDetectorDigitalIn)
    else:
        dwf.FDwfDigitalInTriggerSourceSet(device_handle, trigsrcNone)
